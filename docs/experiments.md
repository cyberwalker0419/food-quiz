...
## 任务⑪ 追问必答量 25→30 + late 启动解耦 — commit（待定）

> 用户报告「追问机制很少触发」。先证伪"阈值太严"假设，再按用户决策提必答量到 30。

### 诊断：阈值非主因（1A 假设证伪）

closestTo 32 runs 基线（MIN=25）：mean 29.1 / pursueRate 56% / dup 0 —— **追问率本就不低**。一致画像（嗜辣/嗜浓）25 题即停，诊断一致嗜辣 seed=1001 停时各维状态：

| 维 | profile | \|w\|累计 | 维 | profile | \|w\|累计 |
|:--|--:|--:|:--|--:|--:|
| sour | 562 | 562 | salty | 718 | 718 |
| sweet | 297 | 297 | rich | 765 | 765 |
| temperature | 280 | 600 | crunchy | 545 | 545 |
| spicy | 720 | 720 | tender | 575 | 575 |

**所有 8 维 |w|累计 ≥ 297**（≫ CLARIFIED_ABS=140 / COVERAGE_FLOOR=180）→ 三机制全 false → 追问集空 → 25 题停。根因：**选项权重多维联动**（选 spicy 高的选项，salty/rich/crunchy/tender 同步推高），closestTo 单调强化让一致画像全维快速饱和。提 CLARIFIED_ABS 到 280 仍被超（profile 280–765），无效。**一致画像不追问是设计正确**（无矛盾/无欠探索可追问），非阈值问题。

### 改动（按用户决策「增加必答题数量到 30」）

1. `MIN_QUESTIONS` 25→30（shouldStop 必答下限）。
2. 新增 `LATE_START=25`，与必答下限解耦：`pickNextQuestion` late 分支（信息增益 + 追问 contraBoost）+ `sharpnessWeight` 早/晚阶层都锚定 LATE_START，不随 MIN 漂移。
3. `adaptiveSelector.test.ts` 冒烟测试「完整 25 题」→「完整 30 题」。

### closestTo 32 runs 对比（5 一致 + 3 摇摆 × 4 seed）

| 配置 | mean | pursueRate | dup | 说明 |
|:--|--:|--:|--:|:--|
| 基线 MIN=25 | 29.1 | 56% | 0 | 原生产配置 |
| MIN=30 单改（弃） | 31.3 | 50%（quiz-sim 44%） | 0 | late 推迟到 count≥30，追问窗口 30–45 被压缩 → pursueRate 跌 |
| **MIN=30 + LATE_START=25 解耦（落地）** | **31.4** | **50%（quiz-sim 47%）** | **0** | 追问窗口回 25–45；必答 30 达成 |

分画像（落地配置）：一致画像 mean 31.4 / pursueRate 45%；摇摆画像 mean 31.4 / pursueRate 58%。

### 结论与取舍

- **必答 30 达成**：mean 29.1→31.4，所有 session ≥ 30 题。
- **pursueRate 微降 56%→50%** 是"多答 5 题→一致画像更饱和"的必然（非 late 推迟所致——解耦后追问窗口已回 25–45，pursueRate 未回升印证此点）。quiz-sim 47% 仍过 ≥40% 护栏；一致画像追问率低是设计正确（无矛盾可追问）。
- **解耦保留**：25–29 题走 late 分支（信息增益选题比 early 均匀覆盖更精准）+ sharpnessWeight 阶层锚定 12/25 不漂移 + 现有 sharpnessWeight 测试零改动通过。
- 选题评分公式未改，acc/conc/earlyCen/lateCen 预期不回归（这些指标针对评分改动），未单独量化。

**1A 阈值假设证伪留档**：detectPursueDims 的 WEAK_W=5/CLARIFIED_ABS=140/INCONSISTENCY_GAP=0.45 非追问少的主因。**可重试条件**：若未来要让一致画像也触发追问，需重新定义追问语义（当前=矛盾/欠探索；一致画像本就不该被追问），或引入 UX 追问可见标识——属另一战线，非调阈值。

三件套绿（270/270 + tsc + build）。

---

## 任务⑫ 1B 机制B 近期窗口收益评估 — 边际，未落地

> 1A 留档"1B 让追问更精准（机制B 改近期窗口判定）"。用户要求"查看收益"后再定 1B 去留。
> 评估方式：临时脚本 `_1b-assess-exp.test.ts`（测完即删）跑 closestTo 32 runs，统计 detectPursueDims 标记的追问维特征 + 对比近期窗口(N=8)机制B 差异。

### 评估结论：收益边际

1. **全 session 追问维 100% 是淡漠维**（|profile|<100，累计 18 个追问维全部如此；如一致嗜辣停@33 的追问维 temperature pf=−34）——主导维（spicy pf=720 等）早已 `|profile|≥CLARIFIED_ABS=140` 退出。追问机制只在"用户未明确表态的淡漠维"上工作，这本身合理。
2. **淡漠维追问主因是机制 C（覆盖度不足：累计<180）**，机制 B（强弱波动）只是补充。例：temperature 被标记主要是机制 C（题库密度 27.7≥25 不跳过 + 累计<180）。
3. **1B 近期窗口只能改机制 B 的贡献**，机制 C 仍兜底标记这些淡漠维 → 改机制 B 为近期窗口后，被剔的维大多仍被机制 C 追问 → **净追问行为几乎不变**。

### 可重试条件

1B 近期窗口要有价值，须先**削弱机制 C 对淡漠维的覆盖度追问**（否则机制 B 改不改都被机制 C 覆盖），或重新定义"淡漠维是否该被追问"（当前为凑覆盖度而问，用户可能感知为"问我不关心的维"——属设计哲学，非算法阈值）。

**未落地**（机制 B 维持全 session 判定）。临时脚本已删验证。

---

## 任务⑬ blendedScore 距离项归一化 — 解决菜系恒定 49% + 推荐菜不准（落地）

> 用户报告「适合菜系恒定 49%」「推荐菜不准确」。诊断（数值验证 5 画像 × 245 菜）：
> `blendedScore = 0.5·cos + 0.5·(1/(1+dist))` 在 [0,100] 量纲下，典型 dist≈60–100 → `1/(1+dist)≈0.01`，×0.5≈0.005，**距离项贡献≈0** → 退化为 `0.5·cos`，`cos=(cen+1)/2∈[0,1]` → **天花板≈0.5**。5 画像最高菜系匹配度全部压在 43–50%（嗜辣 50/嗜甜 46/清淡 43/重口 47/嗜嫩 48），菜分数挤在 0.09–0.50，`MATCH_RATIO=0.6` 让匹配池装下几乎所有菜 → `score²` 加权近随机 → 推荐菜不准。

### 改动（[similarity.ts:59](food-quiz/src/lib/taste/similarity.ts:59)）

距离项归一化到 [0,1]：`1/(1+dist)` → `1 - dist/MAX_DIST`，`MAX_DIST=√8·100≈282.84`（8 维 [0,100] 最大欧氏距离）。`euclideanDist` 函数本身不改（去中心化 dedup/penalty 仍用原值），仅 blendedScore 内归一化。同步 `similarity.test.ts` 三处断言（full-full=0.75 不变；zeros-full 0.25177→0.25；形状相反 0.00177→0）。

### 5 画像 × 153 popular 菜对比（临时脚本 `_2a-blended-exp.test.ts`，测完即删）

| 画像 | 改前最高菜系 | 改后最高菜系 | 改后 top5 菜（语义抽检） | 菜分数极差 |
|:--|--:|--:|:--|--:|
| 嗜辣 | 50% | **89%**（渝菜/川菜） | 万州烤鱼/重庆小面/麻辣烤鱼/钵钵鸡/担担面 ✓ | 0.60 |
| 嗜甜 | 46% | **82%**（西餐/粤菜） | 糖醋排骨/清补凉/冰淇淋/蛋挞/可乐鸡翅 ✓ | 0.51 |
| 清淡全能 | 43% | **79%**（清真菜） | 清真凉皮/西北凉面/凉皮/京酱肉丝(素)/回族素面 ✓ | 0.30 |
| 重口咸浓 | 47% | **85%**（川菜/西北） | 回锅肉/湘西腊肉/胡辣汤/担担面/烤羊肉串 ✓ | 0.50 |
| 嗜嫩清淡 | 48% | **86%**（家常/粤菜） | 番茄炒蛋/豆腐脑/清蒸鱼/青稞粥/蒸凤爪 ✓ | 0.44 |

菜分数 mean 0.59–0.73 / min 0.32–0.53 / max 0.83–0.92；菜系匹配度出现真实梯度（如嗜辣 89/89/87/86/84 → … → 30%）。

### 结论

- **判据全满足**：最高菜系匹配度 79–89%（≥60% ✓）、极差扩大（0.30–0.60 vs 0.20–0.40 ✓）、top5 菜语义全部对路 ✓。
- **不影响选题/dedup**：blendedScore 仅用于推荐菜/菜系匹配；选题去重用 `centeredSig`/`signatureSim`，closestTo 用 `cosineSim`，均不改 → quiz-sim 三数不回归（268/268 绿）。
- 放大因素（菜标注偏态：sour/sweet/temperature/spicy/crunchy 五维 85%+ 菜≤20）留待任务⑭（2B 菜标注错标修补）选择性修正——2A 打开天花板后标注偏态影响已大幅降低。

三件套绿（268/268 + tsc + build）。

---

## 任务⑭ MATCH_RATIO 收窄推荐池 + 菜标注错标扫描（落地 + 评估）

> 2A preview 暴露残留：topCuisines 已准（菜系梯度），但 topDishes 仍抽到"菜系对、该菜不典型"的菜（嗜辣 seed=42 推 5 道仅 1 辣：炸酱面/羊肉泡馍/肠粉/汆白肉/水煮鱼）。

### 2B-① MATCH_RATIO 0.6→0.8（落地）

根因：`MATCH_RATIO=0.6` 过滤池（[result.ts:344](food-quiz/src/lib/taste/result.ts:344)）——2A 后 topScore≈0.85–0.91，0.6 池（score≥0.5）放进大量"主导维不突出但 salty/rich 维匹配"的菜 → `score²` 加权抽到非典型菜。改 0.8 只留 score≥0.68–0.73 的高分典型菜（池<5 退回全量兜底）。

3 画像 × 3 seed 验证（临时脚本 `_2b-ratio-exp.test.ts` 测完即删）：

| 画像 | 改前典型度(seed42) | 改后典型度(3 seed) | 改后 topDishes 示例 |
|:--|--:|--:|:--|
| 嗜辣 | 1/5 | **4/5, 5/5, 4/5** | 蒜蓉小龙虾/辣子鸡/椒麻鸡/酸汤鱼/火锅(seed100 全辣) |
| 嗜甜 | — | **5/5, 4/5, 4/5** | 雪衣豆沙/糖醋排骨/双皮奶/提拉米苏/珍珠奶茶(seed42 全甜) |
| 重口咸浓 | — | **5/5, 5/5, 5/5** | 干煸四季豆/麻辣烫/烤全羊/烩面/辣椒炒肉 |

**问题 3 彻底解决**：topDishes 从"5 道 1 典型"→"4–5/5 典型"，未触发池兜底。三件套绿（268/268 + tsc + build）。

### 2B-② 菜标注错标扫描（评估：质量好，无需校准）

6 规则扫 153 popular 菜（辣菜不辣/甜菜不甜/脆菜不脆/清蒸却脆/嫩菜不嫩/烤炸不脆），命中 5 道：
- **误报 3 道**（标注本就正确）：麻花 spicy=0（不辣，"麻"误匹配）、万州烤鱼/麻辣烤鱼 crunchy=20-30（鱼肉嫩不脆，"烤炸应脆"对烤鱼不对）。
- **真错标 2 道**：蒙古奶酪 sweet=25、蒙古酸奶 sweet=20（甜口奶制品，sweet 偏低）。

**结论**：菜标注质量好（153 道仅 2 道小错标），印证问题 2+3 根因是评分公式（任务⑬）+ 池宽（本任务①），非数据层。2 道小错标增量极小（仅嗜甜画像偶尔抽到），**未修**（改 public 数据 dishes.json 需用户确认；留待用户定是否单独修）。原计划"2B 菜标注校准"因扫描证明无系统性错标而收尾。
