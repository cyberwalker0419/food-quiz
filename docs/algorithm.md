...
**现象**：追问机制 C（覆盖度不足）跳过 `BANK_MIN_DENSITY < 25` 的维度 → 苦维（题库密度 ≈ 14.4）**永不追问**，`COVERAGE_FLOOR`/`CLARIFIED_ABS` 对苦维形同虚设。

**根因**：题库苦维信号密度不足（数据层制约）。`bitter` 初始 −15（§4.2）是补偿，但算法无法弥补"题不够"导致的画像模糊。

**性质**：**数据层制约算法**，算法侧不可解。需扩充苦维题库（content 层）才能让机制 C 覆盖苦维。
