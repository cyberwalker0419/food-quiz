...
food-quiz 是一个**零网络**的「中国味觉性格测试」：被试先选忌口，再作答 30–45 道自适应题目，每题选项携带 8 维味觉权重，累加成味觉画像向量，归一化后渲染为雷达图 + 长评价 + 跨菜系推荐菜 + 国风分享卡。全部数据编译期内联（Vite glob），无 fetch / 无 API / 无后端。
...
profile 是 `WeightVector`（原始，未归一化），可正可负，范围约 ±几百（30–45 题累积）。
...
### 5.6 追问机制（detectPursueDims，渐进式 30–45）
...
### 5.7 停止判定 shouldStop（30–45 渐进模型）

$$count \ge 45 \to \text{停};\quad count < 30 \to \text{继续（必答下限 MIN\_QUESTIONS=30）}$$

$count \ge 30$ 后按残余追问维度数渐进容忍：
...
> **必答下限与 late 启动解耦（任务⑪）**：`MIN_QUESTIONS=30`（shouldStop 必答下限）与 `LATE_START=25`（pickNextQuestion late 分支 + 追问 contraBoost 启动门槛）分离——追问窗口仍是 25–45，提必答量不压追问率。`sharpnessWeight` 早/晚阶层也锚定 `LATE_START`（12/25），不随必答量漂移。

实测平均约 31 题（任务⑪；原 25–45 模型 ~33 题/`93ae269`）。
