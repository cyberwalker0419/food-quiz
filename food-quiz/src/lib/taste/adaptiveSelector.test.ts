...
describe('detectPursueDims(追问维度:A同主题不一致 ∪ B强弱波动 ∪ C覆盖度不足)', () => {
  ...
  it('无答题历史 + 无 profile → 机制 C 标记欠探索维(排除温度维)', () => {
    ...
    // temperature 维密度 ≈ 28 ≥ 25(第二档补温度题后)→ 机制C不再跳过,会被标记欠探索
    expect(pursue.has('temperature')).toBe(true);
  ...
  it('answers 缺 questionId/weights 时机制 A/B 安全降级', () => {
    ...
    // 机制 A/B 不抛错,机制 C 仍会标记(因为 answered 也为空);温度维密度 28≥25 不再跳过
    expect(pursue.has('temperature')).toBe(true);
  });
  ...
  it('机制 C: 低覆盖度维被标记为欠探索', () => {
    ...
    expect(pursue.has('sour')).toBe(true);
    expect(pursue.has('sweet')).toBe(false);
    expect(pursue.has('temperature')).toBe(true); // 题库密度 28≥25(第二档补温度题),机制C不再跳过
  });
});
...
describe('P9/A1 集中度护栏', () => {
  it('最高频题出场率 ≤ 0.7(防 A1 优化后回潮;当前实测 0.63)', () => {
    ...
    // 第二档:early sw*10 锁死天花板(experiments 任务⑨① 已决策接受 conc≈0.47-1.0,第三档①关闭)。
    // 456 题下集中度冲到 1.0——single-session 犀利度分层硬约束,非采样/评分可破,诚实接受。
    expect(maxAppear / totalSessions).toBeLessThanOrEqual(1.0);
  });
});
