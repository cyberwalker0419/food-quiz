...
describe('真实题库端到端模拟 — 目标:平均 30-35 题,机制 B 常触发', () => {
  it('5 个画像(2 一致 + 3 矛盾/波动)各 4 seed,平均应 ∈ [26,36] + 机制B触发率 ≥ 40%', () => {
    ...
    // 温度维(替 bitter) + P10/P11 MMR 去重叠加后基线:平均题数 26-36,机制B 触发率 ≥ 40%
    // (温度维密度 ~25 被机制C跳过;MMR 去重让一致画像收敛更快、减少冗余追问,
    //  仅矛盾/波动画像触发机制B,故触发率较纯温度维 main 基线 45% 再降)
    expect(mean).toBeGreaterThanOrEqual(26);
    expect(mean).toBeLessThanOrEqual(36);
    expect(pursueRate).toBeGreaterThanOrEqual(0.40);
  });
